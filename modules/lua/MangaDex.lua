local function buildLanguageSelection(module)
	...
	local setting = module.Settings.AddChoice("Preferred language(s)", defaultSettingValue, options).WithMultiSelect()
...
function Register()
	...
	if API_VERSION >= 3 then
		...
		module.Settings:AddCheck("Prefer scanlation status over publishing status", false):WithToolTip("If enabled, the scanlation status will be used instead of the publishing status in the metadata.")
	end
