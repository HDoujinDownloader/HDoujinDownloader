...
local function getImageServerFromGalleryId(galleryId)
	...
	elseif galleryId > 1259410 and galleryId <= 1439024 then
		imageServer = "m9"
	elseif galleryId > 1439024 and galleryId <= 1663755 then
		imageServer = "m10"
	else
		imageServer = "m11"
	end
...
function GetInfo()
	...
	if url:contains("/gallery/") then
		...
		local maxScrapingDepth = tonumber(global.GetSetting("Downloads.MaxScrapingDepth"))
		if maxScrapingDepth then
			maxScrapingDepth = 1
		end
